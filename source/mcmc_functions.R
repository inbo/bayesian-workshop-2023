...
MCMC_metro <- function(x, y, N = 20, sigma = 1, init_b0, init_b1){
  ...
  mcmc <- data.frame(iter = 1:N,     # positie in de markov chain
                     beta_0 = NA,    # intercept
                     beta_1 = NA,    # helling
                     tau = NA,       # 1/standaard afwijking
                     post = NA, # posterior
                     accept = NA)    # stap wel (1) of niet (0) geaccepteerd.

  mcmc$beta_0[1] <- init_b0
  mcmc$beta_1[1] <- init_b1
  mcmc$tau[1] <- 0.228  # init_tau voor de eenvoud houden we tau fixed.

  # Berekening van de posterior voor de startwaarde
  mcmc$post[1] <- ll.fun(a = mcmc$beta_0[1], b = mcmc$beta_1[1], tau = mcmc$tau[1]) +
    prior.fun(a = mcmc$beta_0[1], b = mcmc$beta_1[1], tau = mcmc$tau[1])
  ...
  for(i in 2:N){
    ...
    new_post <- ll.fun(a = new_beta_0, b = new_beta_1, tau = new_tau) +
      prior.fun(a = new_beta_0, b = new_beta_1, tau = new_tau)
