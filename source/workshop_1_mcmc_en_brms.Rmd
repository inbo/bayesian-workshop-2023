...
library(patchwork) # figuren combineren
...
In deze sectie leggen we kort enkele basisprincipes uit Bayesiaanse statistiek. 
...
Stel dat we geïnteresseerd zijn in het aantal soorten mieren dat voorkomt in een onderzoekssite van een vooraf gespecifieerde grootte.
Aantallen worden vaak gespecifieerd door een Poisson verdeling aangezien deze verdeling enkel positieve gehele getallen aanneemt. 
...
De figuur hieronder toont dat de kans op $X = 8$ verschillend is in ieder van de voorbeeld likelihoods.
...
Bij statistische inferentie willen we de likelihood eigenlijk inverteren door gebruik te maken van onze voorkennis en voorkennis (prior).
Op deze manier hopen we te weten te komen welk van alle modellen het meeste zinvol/waarschijnlijk is.
...
$$
Dit zorgt er voor dat we voor iedere potentiele waarde van $\theta$ een waarschijnlijkheid zal hebben. De parameters hebben bij Bayesiaanse modellen dus een verdeling. Dit staat in contrast met frequentist inferentie waar de oplossingsruimte voor $\theta$ opgesplitst wordt in twee delen: waarden die niet of wel potentieel de *echte* waarde zijn.

In ons voorbeeld van het aantal soorten mieren is:

- $\theta$ het aantal soorten mieren
- $X$ de data
...
p2 <- ggplot(bimodal) +
  ...
  geom_text(data = measures,
            aes(label = measures, x = x, y = 0.1), color = "red")
p3 <- ggplot(bimodal) +
...
Bij symmetrische, unimodale vverdelingen zijn HPDI en CPI hetzelfde. Anders kan er best wel wat verschil 
