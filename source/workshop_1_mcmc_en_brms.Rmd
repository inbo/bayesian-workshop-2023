...
-$H_0:$ de hypothese $\lambda$ is waar
-$H_1:$ de hypothese $\lambda$ is niet waar

De vuistregel in frequentist statistiek is dan:

- als $P($data $X$ of extremer$|\lambda) < 0.05$, dan is \lambda niet waar  $\Rightarrow  P(\lambda|X) = 0$
- als $P($data $X$ of extremer$|\lambda) \geq 0.05$, dan zou \lambda kunnen waar zijn  $\Rightarrow P(\lambda|X) = ?$

In onderstaande figuur staan de volgende kansen afgebeeld:

- $p(X\geq8|\lambda = 5)\simeq$ `r 1 - round(likelihood %>% filter(x <= 8 & lambda == 5) %>% dplyr::pull(prob) %>% sum(), 3)` Dit is groter dan 0.05 dus we kunnen de hypothese dat $\lambda$ gelijk is aan 5 niet verwerpen.
- $p(X\leq8|\lambda = 10)\simeq$ `r round(likelihood %>% filter(x <= 8 & lambda == 10) %>% dplyr::pull(prob) %>% sum(), 3)` Dit is groter dan 0.05 dus we kunnen de hypothese dat $\lambda$ gelijk is aan 10 niet verwerpen.
- $p(X\leq8|\lambda = 15)\simeq$ `r round(likelihood %>% filter(x <= 8 & lambda == 15) %>% dplyr::pull(prob) %>% sum(), 3)` Dit is kleiner dan 0.05 dus we verwerpen de hypothese dat $\lambda$ gelijk is aan 15.
...
Als we deze berekeningen herhalen voor verschillende waarden van $\lambda$, kunnen we ontdekken dat het 90% confidence interval (5% langs beiden kanten) gelijk is aan $4.0 \leq \lambda \leq 14.4$.
De beslissingsruimte voor de parameter $\lambda$ wordt dus opgedeeld in twee delen; waarden die waarschijnlijk zijn en waarden die niet waarschijnlijk zijn.
...
Dit zorgt er voor dat we voor iedere potentiele waarde van $\lambda$ een waarschijnlijkheid zullen hebben.
...
MCMC staat voor 'Markov chain Monte Carlo'. We gaan niet dieper in op deze begrippen. Gaandeweg wordt wel duidelijk wat hiermee wordt bedoeld.

De posterior distributie kan je vergelijken met een berg. De top van de berg ligt waar de parameterwaarde het meest waarschijnlijk is na het combineren van de likelihood van je data met je prior (maximum likelihood). De ligging en de vorm van de bergtop is echter onbekend. Het doel van een MCMC-algoritme is op een efficiënte wijze de bergtop en de omgeving te verkennen. 

MCMC genereert een reeks van parameterwaarden aan de hand van een aantal regels die de sampler toelaat de hele range van de posterior distributie te exploreren. De regels zorgen ervoor dat de sampler de hele tijd in de buurt van de top van de berg blijft.

MCMC is een random wandeling in de parameterruimte in de omgeving waar de posterior distributie het hoogst is (de top van de berg). De grootte en de richting van de stap varieert.
...
5. Is de likelihood voor de nieuwe posterior $>$ huidige posterior $\Rightarrow$ nieuwe waarde behouden
6. Is de likelihood voor de nieuwe posterior $<$ huidige posterior $\Rightarrow$
...
Het is doordat de kans om parameterwaarden met een lagere likelihood te selecteren varieert met de ratio van de likelihoods dat MCMC in de limiet (na heel veel iteraties) een correcte afspiegeling is van de posterior distributie. 
...
```{r}
mcmc %>% 
  ggplot(aes(x = iter, y = a)) +
  annotate("rect",  xmin = 0, xmax = 200, ymin = -Inf, ymax = +Inf,
           alpha = 0.3, fill = "gold") +
  annotate("label", label = "burn-in", x = 250, y = 12, hjust = "left",
           fill = "gold") +
  geom_line()
```
