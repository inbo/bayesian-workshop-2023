...
Er zijn verschillende manieren om convergentie van het MCMC algoritme voor elke parameter te te controleren.
Eerst en vooral heb je visuele controles.

We kunnen de MCMC samples met de `as_draws()` functies verkrijgen ofwel via de [bayesplot](https://mc-stan.org/bayesplot/) package die compatibel is met brmsfit objecten.
...
Gelijkaardig als een trace plot kunnen we ook kijken hoe bepaalde statistieken zoals het gemiddelde of de quantielen zich voordoen naarmate het aantal iteraties toeneemt.
...
> uitleg Wat is posterior density?
...
> uitleg Wat is autocorrelatie?

```{r}
mcmc_acf(fit_normal1, pars = parameters)
```

Andere packages die van pas kunnen komen zijn [mcmcplots](https://cran.r-project.org/web/packages/mcmcplots/index.html) en [coda](https://cran.r-project.org/web/packages/coda/index.html).
...
Eén manier om te controleren of een chain is geconvergeerd, is door zijn gedrag te vergelijken met andere willekeurig geïnitialiseerde ketens.
...
Als alle ketens convergeren naar een gemeenschappelijke distributie, zullen deze verhoudingen één zijn. Als de ketens niet zijn geconvergeerd naar een gemeenschappelijke verdeling, zal de $\hat{R}$ statistiek groter zijn dan één.
...
Het idee achter de posterior predictive check is dat, als een model goed past, de voorspelde waardes o.b.v. het model veel lijken op de data die we hebben waargenomen (en gebruikt om het model te fitten).
...
Ze word vaak gebruikt voor het modelleren van aantallen gedurende een gegeven tijdsinterval, afstand, oppervlakte, volume etc.
...
fit_normal1 <- add_criterion(fit_normal1,
                             criterion = c("loo", "kfold", "waic"), K = 10)
