...
- als $P($data $X$ of extremer$|\lambda) \geq 0.05$, dan zou \lambda kunnen waar zijn  $\Rightarrow P(\lambda|X) = \text{?}$

In onderstaande figuur staan de volgende kansen afgebeeld:

- $p(X\geq8|\lambda = 5)\simeq$ `r 1 - round(likelihood %>% filter(x <= 8 & lambda == 5) %>% dplyr::pull(prob) %>% sum(), 3)`\
  Dit is groter dan 0.05 dus we kunnen de hypothese dat $\lambda$ gelijk is aan 5 niet verwerpen.
- $p(X\leq8|\lambda = 10)\simeq$ `r round(likelihood %>% filter(x <= 8 & lambda == 10) %>% dplyr::pull(prob) %>% sum(), 3)`\
  Dit is groter dan 0.05 dus we kunnen de hypothese dat $\lambda$ gelijk is aan 10 niet verwerpen.
- $p(X\leq8|\lambda = 15)\simeq$ `r round(likelihood %>% filter(x <= 8 & lambda == 15) %>% dplyr::pull(prob) %>% sum(), 3)`\
  Dit is kleiner dan 0.05 dus we verwerpen de hypothese dat $\lambda$ gelijk is aan 15.
...
Dit zal de autocorrelatie reduceren maar natuurlijk ook het totaal aantal iteraties ... 
